...
// echo $email;
// die;
$sql_login = tampil("SELECT `tbl_petugas`.*, `tbl_users`.*, `tbl_tipe_user`.* FROM `tbl_petugas` LEFT JOIN `tbl_users` ON `tbl_petugas`.`id_user` = `tbl_users`.`id_user` LEFT JOIN `tbl_tipe_user` ON `tbl_users`.`role` = `tbl_tipe_user`.`id_tipe_user` WHERE tbl_users.email='$email'");
// var_dump($sql_login);

foreach ($sql_login as $user_login) {
    $nama_user = $user_login['nama_petugas'];
    $photo = $user_login['path_photo_petugas'];
					...
					<div class="header-actions">
						<div class="dropdown ms-3">
							<a id="userSettings" class="dropdown-toggle d-flex py-2 align-items-center text-decoration-none"
								...
								<img src="../images/petugas/<?= $photo; ?>" class="rounded-circle img-3x" alt="Bootstrap Gallery" />
